...
- A `user-agent: anthropic-community-elixir/<version>` header is now sent with every request (superseding [#1](https://github.com/tubedude/anthropic-community/pull/1), which targeted the now-deleted pipeline-era `HTTPClient.Utils`; this version lives in `HTTPTransport.base_headers/1`, the single place already shared by every resource, streaming included).
